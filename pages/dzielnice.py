...
def create_top6_district_chart(df, selected_city):
    ...
    if df is None or df.empty or 'district' not in df.columns:
        st.warning("⚠️ Brak kolumny 'district' (dzielnica) w przetworzonych danych.")
        return None

    # Kopia danych i pełna normalizacja tekstowa
    df_clean = df.copy()
    ...
    if city_df.empty:
        st.info(f"ℹ️ Brak dostępnych ofert z ceną za m² dla miasta: {selected_city}")
        return None

    # Obliczanie średniej ceny za m² w podziale na dzielnice
    stats = city_df.groupby('district')['price_per_m2'].mean().reset_index()
    stats = stats.rename(columns={'district': 'Dzielnica', 'price_per_m2': 'Średnia cena (zł/m²)'})

    # Oczyszczanie z wartości pustych oraz placeholderów systemowych
    stats['Dzielnica_clean'] = stats['Dzielnica'].astype(str).str.lower().str.strip()
    stats = stats[
    ...
    stats = stats.sort_values(by='Średnia cena (zł/m²)', ascending=True)

    if stats.empty:
        st.warning(f"⚠️ Wszystkie znalezione dzielnice dla miasta {selected_city} to wartości puste lub 'Nieznane'.")
        return None
    ...
    fig = px.bar(
        ...
        y='Dzielnica',
        orientation='h',
        title=f"🏙️ Średnia cena za m² według dzielnic w mieście: {selected_city}",
        labels={'Średnia cena (zł/m²)': 'Średnia cena (zł/m²)', 'Dzielnica': 'Dzielnica'},
        color='Średnia cena (zł/m²)',
    ...
    fig.update_layout(
        ...
        xaxis_title="Średnia cena (zł/m²)",
        yaxis_title="Dzielnica"
    )
    return fig

def main():
    st.title("🏙️ Analiza Dzielnic TOP 6")
    st.markdown("Dedykowany moduł porównania średnich cen nieruchomości za $m^2$ w obrębie dzielnic największych polskich miast.")
    st.divider()
    ...
    if df is None or df.empty:
        st.error("❌ Brak danych w bazie nieruchomości dla Twojego konta.")
        return

    # --- AGRESYWNE OCZYSZCZANIE NAZW MIAST (BEZPIECZEŃSTWO) ---
    df['city'] = df['city'].astype(str).str.replace(r'[\r\n\t]+', '', regex=True).str.strip()
    
    # [POPRAWKA]: Definicja zmiennej na samym początku procesu oczyszczania danych
    ...
    if not available_top6:
        st.warning("⚠️ W bazie danych nie znaleziono żadnych ofert powiązanych z miastami TOP 6.")
        st.info(f"Twoja baza zawiera obecnie miasta: {raw_cities_in_db}")
        return
    ...
    selected_city = st.selectbox(
        "Wybierz miasto do analizy dzielnicowej:", 
    ...
    if st.sidebar.button("💾 Zapisz to wyszukiwanie", use_container_width=True):
        try:
            ...
            save_search(username, selected_city, "[TOP6] Wszystkie dzielnice")
            st.toast(f"✅ Zapisano analizę dla miasta {selected_city}!")
            st.rerun()
    ...
    st.subheader(f"📊 Wyniki dla miasta: {selected_city}")
    
    fig = create_top6_district_chart(df, matched_db_city)
    if fig is not None:
        st.plotly_chart(fig, use_container_width=True)
        
        with st.expander("🔍 Zobacz zestawienie tabelaryczne"):
            ...
            if not city_df_clean.empty:
                ...
                table_stats.columns = ['Dzielnica', 'Śr. cena (zł/m²)', 'Liczba dostępnych ofert']
                st.dataframe(table_stats.style.format({'Śr. cena (zł/m²)': '{:.2f} zł'}), use_container_width=True, hide_index=True)
            else:
                st.write("Brak szczegółowych danych o dzielnicach dla tego miasta.")
    else:
        st.warning("Nie można wygenerować wykresu słupkowego (zbyt mało danych szczegółowych o dzielnicach w bazie).")

    # --- INTERAKTYWNA HISTORIA WYSZUKIWAŃ (ODSEPAROWANA DLA TOP 6) ---
    st.divider()
    st.subheader("🕒 Twoja historia wyszukiwań dla TOP 6")
    ...
    if history_raw:
        ...
        # Filtrujemy dane tak, aby pokazać wyłącznie rekordy z tagiem [TOP6]
        ...
        if not recent_searches.empty:
            ...
            with col_h2: st.markdown("**Miasto**")
            with col_h3: st.markdown("**Szczegóły**")
            ...
            for idx, row in recent_searches.iterrows():
                ...
                with c4:
                    if first_city_match in available_top6:
                        if st.button("Wczytaj 🔄", key=f"load_btn_{idx}", use_container_width=True):
                            st.session_state["top6_selected_city"] = first_city_match
                            st.toast(f"🔄 Załadowano filtry dla: {first_city_match}")
                            st.rerun()
                    else:
                        st.caption("Ogólne menu")
        else:
            st.info("Brak zapisanych wyszukiwań dla modułu TOP 6.")
    else:
        st.info("Brak zapisanych wyszukiwań w historii dla Twojego profilu.")
