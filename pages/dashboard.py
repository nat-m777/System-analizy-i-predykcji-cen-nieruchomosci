...
def mark_outliers(group):
    """Oznacza okazje i oferty premium na podstawie rozkładu IQR."""
...
def render_dashboard_ui(df, sel_cities, sel_districts, avg_val, history_df):
    """Główny interfejs Dashboardu - wersja bez dodatkowego wykresu miast."""
    ...
    with tabs[0]:
        ...
        if len(df_charts) > 1:
            # Pozostawiamy tylko główne wykresy z modułu charts.py
...
def main():
    ...
    # 1. Pobieranie danych i historii
    df_raw = db.get_all_offers(username)
    df = clean_df(df_raw)
    
    # Pobieramy historię wyszukiwań dla aktualnego użytkownika
    try:
